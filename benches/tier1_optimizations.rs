...
use octaindex3d::performance::arch_optimized::{
    has_bmi2, batch_morton_encode_bmi2, batch_morton_decode_bmi2
};

use octaindex3d::performance::fast_neighbors::{
    neighbors_route64_fast, batch_neighbors_fast, batch_neighbors_auto,
    batch_neighbors_medium,
};
...
fn bench_morton_encoding_comparison(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000] {
        ...
        group.bench_with_input(
            ...
            |b, coords| {
                b.iter(|| {
                    let result: Vec<u64> = coords.iter()
        ...
        if has_bmi2() {
            group.bench_with_input(
                BenchmarkId::new("bmi2", size),
                &coords,
                |b, coords| {
                    b.iter(|| {
                        black_box(batch_morton_encode_bmi2(black_box(coords)))
                    });
                },
            );
        }
...
fn bench_morton_decoding_comparison(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000] {
        ...
        group.bench_with_input(
            ...
            |b, codes| {
                b.iter(|| {
                    let result: Vec<(u16, u16, u16)> = codes.iter()
        ...
        if has_bmi2() {
            group.bench_with_input(
                ...
                |b, codes| {
                    b.iter(|| {
                        black_box(batch_morton_decode_bmi2(black_box(codes)))
                    });
                },
            );
        }
    ...
    for size in [10, 100, 1000, 10000] {
        ...
        #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
        {
            group.bench_with_input(
                ...
                |b, routes| {
                    b.iter(|| {
                        black_box(batch_neighbors_fast(black_box(routes)))
                    });
                },
            );
        }
        ...
        group.bench_with_input(
            ...
            |b, routes| {
                b.iter(|| {
                    black_box(batch_neighbors_auto(black_box(routes)))
                });
            },
        );
    }
...
fn bench_cache_optimization(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(
            ...
            |b, routes| {
                b.iter(|| {
                    black_box(batch_neighbors_medium(black_box(routes)))
                });
            },
        );
    }
...
fn bench_single_route_neighbors(c: &mut Criterion) {
    ...
    c.benchmark_group("single_neighbor")
        .bench_function("standard", |b| {
            b.iter(|| {
                black_box(neighbors::neighbors_route64(black_box(route)))
            });
        })
        .bench_function("fast_unrolled", |b| {
            b.iter(|| {
                black_box(neighbors_route64_fast(black_box(route)))
            });
        });
}

...
### Fixed
- CI: `security.yml`'s `Security Workflow Audit` job ran `cargo install cargo-audit` without `--locked`, so it resolved the newest semver-compatible version of every transitive dependency on each run. `kstring` 2.0.3 (a transitive dependency of `cargo-audit`) started requiring a Rust toolchain newer than the one pinned by this workflow's `dtolnay/rust-toolchain` step, breaking the job on every PR. Added `--locked` so the install uses `cargo-audit`'s own checked-in lockfile, matching the `cargo-outdated` job's existing pattern.

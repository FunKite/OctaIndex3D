...
### Changed
- Updated `wgpu` from 29.0.3 to 30.0.0 (PR #155): a major release with breaking API changes. This crate only uses `wgpu` for compute pipelines (no vertex/render pipelines), so the widely-publicized `VertexState`/`@interpolate` breaks didn't apply; `wgpu_backend.rs` needed two small adaptations instead — `RequestAdapterOptions` gained a required `apply_limit_buckets` field (set `false`, matching the prior default behavior), and `BufferSlice::get_mapped_range()` now returns `Result<BufferView, MapRangeError>` instead of `BufferView`. The full CI matrix (Linux/macOS/Windows, `gpu-vulkan`/`gpu-metal` feature builds) runs green.

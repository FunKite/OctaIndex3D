...
- Refreshed transitive dependencies in `Cargo.lock` (PR #181, targeting 0.5.9) with `cargo update` under `CARGO_RESOLVER_INCOMPATIBLE_RUST_VERSIONS=fallback`, so every locked crate stays compatible with the 1.90 MSRV. This covers 109 semver-compatible updates that Dependabot's direct-dependency groups do not reach, including `wgpu-core`/`wgpu-hal`/`wgpu-types`/`naga` 30.0.0 → 30.0.1, `rand` 0.9.3 → 0.9.5 and 0.10.2 → 0.10.3, `regex` 1.12 → 1.13, `uuid` 1.18 → 1.26, and `tempfile` 3.23 → 3.27. Duplicate `windows-sys`/`windows-targets` versions are collapsed. `Cargo.toml` is unchanged; Cargo Deny bans/licenses/sources, Clippy, and the local test suite pass.
...
- Updated `bytemuck` from 1.25.0 to 1.25.1 and `zerocopy` from 0.8.52 to 0.8.54 in the `rust-dependencies` group (PR #160): lockfile-only patch bumps with no public API impact.
- Updated `EmbarkStudios/cargo-deny-action` from 2.0.20 to 2.1.1 (PR #159): fixes a `use-git-cli` deprecation warning; no change to `cargo-deny` check semantics.
- Updated `softprops/action-gh-release` from 3.0.1 to 3.0.2 (PR #158): upstream release-reliability and diagnostics fixes; no change to this repository's release workflow behavior.
- Updated `wgpu` from 29.0.3 to 30.0.0 (PR #155): a semver-major upstream release. `RequestAdapterOptions` gained a required `apply_limit_buckets` field (set `false`, matching both the crate's own `Default` impl and this backend's prior behavior), and `BufferSlice::get_mapped_range()` now returns `Result<BufferView, MapRangeError>` instead of `BufferView`; `WgpuBackend` was updated for both changes. The full CI matrix, including the Vulkan and Metal GPU feature-test jobs, ran green.
...
### Fixed
- CI: `security.yml`'s `Security Workflow Audit` job and `rust.yml`'s `Rust Security Audit` job both ran `cargo install cargo-audit` without `--locked`, so each resolved the newest semver-compatible version of every transitive dependency on every run. `kstring` 2.0.3 (a transitive dependency of `cargo-audit`) started requiring a Rust toolchain newer than the one pinned by these workflows' `dtolnay/rust-toolchain` steps, breaking both jobs on every PR. Added `--locked` to both installs so they use `cargo-audit`'s own checked-in lockfile, matching the `cargo-outdated` job's existing pattern.
...
### Security
- Fixed `crossbeam-epoch` at 0.9.18, pulled in transitively via `rayon` (PR #155): upgraded to 0.9.20 (semver-compatible, lockfile-only) to resolve RUSTSEC-2026-0204, an invalid-pointer-dereference bug in its `fmt::Pointer` impl for `Atomic`/`Shared`, published 2026-07-06. `cargo audit` now reports only the pre-existing acknowledged `paste` warning.
